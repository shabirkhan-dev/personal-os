...
# Browser UI test (Personal OS web)
...
Requires a `playwright` MCP server (`@playwright/mcp`). This repo ships no MCP config, so
register that server in your own client before using this path.
