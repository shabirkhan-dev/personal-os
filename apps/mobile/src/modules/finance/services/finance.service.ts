...
export const financeService = {
	...
	getTransactions(query: TransactionsQuery = {}): Promise<FinanceTransaction[]> {
	...
	createTransaction(input: CreateTransactionInput): Promise<FinanceTransaction> {
		return apiClient.post<FinanceTransaction>("/finance/transactions", input);
	},
	...
	deleteTransaction(id: string): Promise<{ id: string; deleted: boolean }> {
		return apiClient.delete<{ id: string; deleted: boolean }>(
			`/finance/transactions/${encodeURIComponent(id)}`,
		);
	},

	getBudgets(month: string): Promise<FinanceBudget[]> {
		return apiClient.get<FinanceBudget[]>(`/finance/budgets/${encodeURIComponent(month)}`);
	},

	setBudgets(month: string, input: SetBudgetsInput): Promise<FinanceBudget[]> {
